...
\usepackage{color}
\usepackage{graphicx}
\graphicspath{{figures/}{../simulations/}}
\usepackage{subfig}
...
\caption{\novo{Fluxograma das etapas do método SMS tanto para regressão quanto para classificação binária. Adaptado de \citeonline{Oliveira2015}.}}
...
\hline
Interseção SMS Linear        & ---   &  0 & 0 & 2 & 2 \\
Interseção SMS Radial        & 0,001 &  0 & 0 & 1 & 1 \\
Interseção SMS Radial        & 0,01  &  0 & 0 & 2 & 2 \\
\textbf{Interseção SMS Radial} & \textbf{0,1} & \textbf{27} & \textbf{6} & \textbf{2} & \textbf{2} \\
\textbf{Interseção SMS Radial} & \textbf{1}   & \textbf{6}  & \textbf{3} & \textbf{2} & \textbf{2} \\
\hline
\multicolumn{6}{L{13.5cm}}{\footnotesize$^{\text{a}}$~Número total de SNPs únicos selecionados pelo SMS.} \\
\multicolumn{6}{L{13.5cm}}{\footnotesize$^{\text{b}}$~Número de SNPs causais entre os selecionados (máx.\ 8). Interseção ``Este trabalho'': SNPs presentes em \textbf{todas} as 10 execuções do kernel; Linear e Radiais $\gamma \in \{0{,}01;\,0{,}1;\,1\}$ retiveram SNP1 e SNP2; Radial $\gamma=0{,}001$ reteve apenas SNP1.} \\
\hline
\end{tabular}
\end{table}
...
\novo{\textbf{Interseção por kernel.} O contraste com Oliveira (2015) é ainda mais marcante na interseção. Em Oliveira, apenas dois kernels produziram interseção não-vazia: Radial $\gamma=0{,}1$ (27 SNPs, 6 causais) e Radial $\gamma=1$ (6 SNPs, 3 causais); os demais três kernels resultaram em interseção vazia --- nenhum SNP foi selecionado em \emph{todas} as 10 execuções, evidenciando alta instabilidade algorítmica. Neste trabalho, \textbf{todos os cinco kernels} produziram interseção não-vazia: Linear, Radial $\gamma=0{,}01$, $\gamma=0{,}1$ e $\gamma=1$ retiveram SNP1 e SNP2 (2 SNPs, 2 causais, precisão 100\%); Radial $\gamma=0{,}001$ reteve apenas SNP1 (1 SNP causal, precisão 100\%). Esse resultado demonstra que a combinação de SMOTE-NC, validação cruzada estratificada e F1 da classe minoritária como \textit{fitness} confere substancialmente maior \textbf{estabilidade algorítmica} ao pipeline: os SNPs que emergem na interseção são exclusivamente causais, constituindo um núcleo duro e confiável de marcadores reprodutíveis entre execuções.}
...
Ambas as abordagens recuperaram principalmente os marcadores com sinal mais forte ou mais estável no ranqueamento. Neste trabalho, a união dos 5 kernels identificou SNP1, SNP2 e SNP5: SNP1 e SNP2 correspondem a efeitos aditivos isolados com $\beta_1=2{,}0$ e $\beta_2=1{,}3$, enquanto SNP5 participa da interação de ordem~2 SNP4--SNP5, associada ao coeficiente $\beta_4=2{,}0$ no modelo da Simulação~6. A interseção convergiu para SNP1 --- o SNP aditivo isolado de maior efeito ($\beta_1=2{,}0$). A interação de ordem~2 não foi recuperada de forma completa, pois SNP4 não aparece conjuntamente com SNP5 na síntese principal, e os SNPs da interação de ordem~3 (SNPs~6--8, coeficiente $\beta_5=3{,}0$) também não foram recuperados como bloco. Esse padrão reflete o desafio inerente à detecção de efeitos epistáticos com poucos controles disponíveis (138 amostras da classe minoritária) e é coerente com o relatado em \citeonline{Oliveira2015}, onde o kernel Radial $\gamma=1$ foi o único capaz de recuperar todos os 8 causais, e apenas após agregar os resultados de 10 execuções.}
...
A Tabela~\ref{tab:comparacao_sim6} apresenta inicialmente os resultados de \textbf{uma única execução} deste trabalho em face da \textbf{união de 10 execuções} de \citeonline{Oliveira2015}. Essa assimetria favorece Oliveira em sensibilidade --- a união de múltiplas execuções inclui naturalmente mais SNPs, aumentando a probabilidade de capturar os causais --- mas penaliza sua precisão, pois acumula também mais falsos positivos. Para eliminar essa diferença de escala experimental, as Tabelas~\ref{tab:sim6_10bases}, \ref{tab:sim6_equivalente_oliveira}, \ref{tab:sim6_tab825} e~\ref{tab:sim6_uniao_1base} apresentam as versões já consolidadas com 10 execuções deste trabalho, distinguindo o protocolo com 10 bases independentes daquele com 1 base fixa e 10 execuções algorítmicas, diretamente comparável ao desenho de \citeonline{Oliveira2015}.}
...
\novo{\textbf{(i) Concordância nos marcadores aditivos de maior efeito.} SNP1 e SNP2, que concentram os dois maiores efeitos aditivos isolados da Simulação~6 (coeficientes $\beta_1=2{,}0$ e $\beta_2=1{,}3$), são ranqueados em 1\textordmasculine{} e 2\textordmasculine{} lugar por ambos os índices, com margem expressiva em relação aos demais. Nesse caso, o sinal genético é suficientemente forte para superar o desequilíbrio de classes.}

\novo{\textbf{(ii) Divergência para marcador epistático de efeito moderado: SNP5.} O SNP5, componente da interação de ordem~2 SNP4--SNP5 associada ao coeficiente $\beta_4=2{,}0$, é o terceiro mais importante segundo o pVI (posição~3, valor~8,254), mas o gVI o relega à posição~29 (valor~2,359). Essa discrepância é característica do viés do gVI em dados desbalanceados: o SNP5 contribui para um sinal epistático relevante à separação da classe minoritária (controles), e divisões baseadas nele isoladamente podem produzir reduções de Gini modestas numa floresta dominada por 862 casos. O pVI, ao permutá-lo e medir a queda de acurácia OOB, capta diretamente seu poder preditivo no modelo ajustado.}
...
\begin{description}
  ...
  \item[\novo{6. Aplicação do pVI como critério de ranqueamento no pipeline SMS:}]
...
\novo{A aplicação das melhores técnicas identificadas --- SMOTE-NC, $k$-fold estratificado e F1 da classe minoritária como \textit{fitness} do GA --- sobre a Simulação~6 de \citeonline{Oliveira2015} produziu conjuntos de SNPs substancialmente mais compactos do que os do método original, mesmo numa base com razão de desequilíbrio de 6,25:1 (862 casos / 138 controles). No protocolo equivalente ao de Oliveira (1~base fixa, 10~execuções algorítmicas), os tamanhos das uniões por kernel foram de 18--31~SNPs, contra 36--99~SNPs no trabalho original --- redução de 60--72\%. A precisão ao nível das variáveis foi superior em todos os kernels (14,8--16,7\% vs.\ 6,1\%--22,2\%), ao custo de menor sensibilidade (37,5--62,5\% vs.\ 62,5--100\%). O ganho em estabilidade algorítmica é ainda mais expressivo na \textbf{interseção}: enquanto Oliveira obteve interseção não-vazia em apenas 2 dos 5 kernels, neste trabalho \emph{todos} os cinco kernels produziram interseção não-vazia, composta exclusivamente por SNPs causais (SNP1 e SNP2 em quatro kernels; SNP1 no quinto) --- precisão 100\% no núcleo estável. Esse resultado evidencia que a adoção do F1 da classe minoritária como critério de seleção orienta o GA para marcadores biologicamente mais relevantes, gerando conjuntos mais parcimoniosos, mais consistentes entre execuções e com maior confiança de relevância, o que é especialmente valioso para validação experimental subsequente.}
