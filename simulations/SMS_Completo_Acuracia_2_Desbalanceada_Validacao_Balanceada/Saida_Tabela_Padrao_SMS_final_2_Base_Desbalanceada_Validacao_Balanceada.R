...
# ------------------------------------------------------------------
# Função de validação cruzada balanceada (estratificada) p/ classificação
# Garante que cada fold mantenha a proporção de classes do conjunto original
...
validacao_cruzada_cls <- function(df, folds = 10, gamma = 0.01, cost = 1.0, epsilon = 0.1, kernel = c("linear","radial")) {
  ...
  idx <- numeric(n)
  classes <- levels(df$fenotipo)
  
  for (classe in classes) {
    classe_idx <- which(df$fenotipo == classe)
...
# Saída LaTeX
writeLines(latex_content, "snps_output_1_GRUPO_2_CLASSIFICACAO_SVM_Desbalanceada.tex")
cat("Arquivo 'snps_output_1_GRUPO_2_CLASSIFICACAO_SVM.tex' foi gerado com sucesso.\n")
