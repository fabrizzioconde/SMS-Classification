...
packages <- c("scrime","e1071","kernlab","randomForest", 
              "doParallel", "GA", "ggplot2", "smotefamily")
...
library(scrime)  # Necessário para simulateSNPglm
library(smotefamily)  # Necessário para SMOTE
...
# Salvando dados originais (antes do SMOTE) em CSV para referência
# Garante que fenotipo seja numérico para facilitar leitura posterior
dados_original_para_salvar <- dados[[1]]
if (is.factor(dados_original_para_salvar$fenotipo)) {
  dados_original_para_salvar$fenotipo <- as.numeric(as.character(dados_original_para_salvar$fenotipo))
}
write.csv(dados_original_para_salvar, file = "dados_original_antes_SMOTE.csv", row.names = FALSE)
cat("Arquivo 'dados_original_antes_SMOTE.csv' salvo com", nrow(dados_original_para_salvar), "observações.\n")
cat("Distribuição salva:\n")
print(table(dados_original_para_salvar$fenotipo))
cat("\n")
...
#------------------------------------------------------------
# Aplicação de SMOTE para balancear a classe minoritária
#------------------------------------------------------------
cat("Distribuição ANTES do SMOTE:\n")
print(table(dados[[1]]$fenotipo))

# Prepara os dados para SMOTE (fenotipo deve ser numérico 0/1)
dados_original <- dados[[1]]
dados_original$fenotipo <- as.numeric(as.character(dados_original$fenotipo))

# Identifica qual é a classe minoritária e majoritária
tab_fenotipo <- table(dados_original$fenotipo)
classe_minoritaria <- as.numeric(names(tab_fenotipo)[which.min(tab_fenotipo)])
classe_majoritaria <- as.numeric(names(tab_fenotipo)[which.max(tab_fenotipo)])
n_minoritaria <- min(tab_fenotipo)
n_majoritaria <- max(tab_fenotipo)

cat("\nClasse minoritária:", classe_minoritaria, "com", n_minoritaria, "observações\n")
cat("Classe majoritária:", classe_majoritaria, "com", n_majoritaria, "observações\n")

# Calcula quantas amostras sintéticas precisam ser criadas
# Para balancear completamente: n_synthetic = n_majoritaria - n_minoritaria
n_synthetic <- n_majoritaria - n_minoritaria

# Calcula dup_size: número de vezes que cada amostra minoritária será aumentada
# dup_size = (n_majoritaria - n_minoritaria) / n_minoritaria
# Arredondado para cima para garantir balanceamento completo
dup_size <- ceiling((n_majoritaria - n_minoritaria) / n_minoritaria)

cat("Criando", n_synthetic, "amostras sintéticas (dup_size =", dup_size, ")\n")

# Separa features e target
X <- dados_original[, setdiff(names(dados_original), "fenotipo"), drop = FALSE]
y <- dados_original$fenotipo

# Aplica SMOTE
# K é o número de vizinhos mais próximos (padrão = 5)
# dup_size controla quantas amostras sintéticas são criadas por amostra original
set.seed(123)
smote_result <- SMOTE(X = X, target = y, K = 5, dup_size = dup_size)

# Extrai os dados balanceados
dados_balanceados <- smote_result$data

# Renomeia a coluna de classe para 'fenotipo'
colnames(dados_balanceados)[ncol(dados_balanceados)] <- "fenotipo"

# Converte fenotipo de volta para fator
dados_balanceados$fenotipo <- factor(dados_balanceados$fenotipo, levels = c(0, 1))

# Substitui dados[[1]] pela versão balanceada
dados[[1]] <- dados_balanceados

cat("\nDistribuição DEPOIS do SMOTE:\n")
print(table(dados[[1]]$fenotipo))
cat("\nTotal de observações após SMOTE:", nrow(dados[[1]]), "\n\n")
...
validacao_cruzada_cls <- function(data, folds = 5, gamma = 0.01, cost = 1, kernel = c("radial","linear")) {
  ...
  # Validação cruzada balanceada (estratificada)
  # Separa os índices por classe para garantir proporção balanceada em cada fold
  idx <- numeric(n)
  ...
  for (classe in classes) {
    ...
    # Distribui os índices da classe entre os folds de forma balanceada
    fold_assignments <- rep(1:folds, length.out = n_classe)
    # Atribui os índices aos folds
    for (j in 1:n_classe) {
...
p <- ggplot(data_grafico_1, aes(x = Geracao, y = Aptidao, group = Estatisticas)) +
  geom_line(aes(colour = Estatisticas, linetype = Estatisticas), linewidth = 2) +
  geom_point() +
...
sink("my_output_SMS_CLASSIF_ACC_1.txt")
