# ==============================================================================
# SMS aplicado à Simulação 6 de Oliveira (2015) — 10 EXECUÇÕES INDEPENDENTES
# ------------------------------------------------------------------------------
...
#
# COMPARAÇÃO JUSTA COM OLIVEIRA (2015):
#   Este script replica a estrutura de 10 execuções independentes de Oliveira (2015).
#   Em cada execução, uma nova base é simulada com semente distinta.
#   Ao final, a UNIÃO dos SNPs selecionados pelo GA é calculada por kernel,
#   exatamente como Oliveira reportou seus resultados (Seção 8.8).
# ==============================================================================
...
n_exec     <- 10
seeds_exec <- 42 + (0:(n_exec - 1))  # seeds 42..51 para as 10 execuções

# ==============================================================================
# SEÇÃO 3 – ARMAZENAMENTO DE RESULTADOS
# ==============================================================================

# snps_GA_all[[exec]][[kernel_nome]] = vetor de SNPs selecionados pelo GA
snps_GA_all    <- vector("list", n_exec)
# metricas_exec[[exec]][[kernel_nome]] = list(corte=..., ga=...)
metricas_exec  <- vector("list", n_exec)
# corte_exec[[exec]][[kernel_nome]] = ponto de corte (inteiro)
corte_exec     <- vector("list", n_exec)

# Timers acumulados
t_sim_vec  <- numeric(n_exec)
...
# ==============================================================================
# SEÇÃO 4 – LOOP PRINCIPAL: 10 EXECUÇÕES INDEPENDENTES
# ==============================================================================

for (exec in seq_len(n_exec)) {
  seed_exec <- seeds_exec[exec]
  cat(sprintf("\n%s\n", strrep("=", 70)))
  cat(sprintf("  EXECUÇÃO %d / %d  (semente base: %d)\n", exec, n_exec, seed_exec))
  cat(sprintf("%s\n", strrep("=", 70)))

  # ---- 4.1 Simular nova base ------------------------------------------------
  cat(sprintf("  Simulando base (semente=%d)...\n", seed_exec))
  set.seed(seed_exec)
  t_sim_ini <- proc.time()
  res_sim <- simulate_scrime_binary_with_ratio(
  ...
  dados_base <- subset(res_sim$data, select = c(-linpred, -prob))
  t_sim_vec[exec] <- round((proc.time() - t_sim_ini)[["elapsed"]])
  cat(sprintf("  [TIMER] Simulação exec %d: %.1f s | Casos: %d | Controles: %d\n",
              exec, t_sim_vec[exec],
              sum(dados_base$fenotipo == 1), sum(dados_base$fenotipo == 0)))

  write.csv(dados_base,
            file = sprintf("dados_sim6_exec%02d.csv", exec),
            row.names = FALSE)

  # ---- 4.2 Random Forest ----------------------------------------------------
  cat("  Executando Random Forest (ntree=4000)...\n")
  data_rf <- dados_base
  if (!is.factor(data_rf$fenotipo)) data_rf$fenotipo <- factor(data_rf$fenotipo, levels = c(0, 1))
  set.seed(exec)
  t_rf_ini <- proc.time()
  ...
  cat(sprintf("  [TIMER] RF exec %d: %.1f s | Top-3: %s\n",
              exec, t_rf_vec[exec],
  ...
  for (ki in seq_along(kernels_config)) {
    ...
    # Salvar gráfico de corte (apenas da execução 1, para não gerar 50 PDFs)
    if (exec == 1) {
      ...
      plot(xs, mean_f1_curve, type = "o", lwd = 2,
           ...
           main = sprintf("Oliveira 2015 – Sim6 (exec 1) | Kernel %s | F1 classe 0", nome))
    ...
    cat(sprintf("    [TIMER] Corte: %.1f s | Corte: %d SNPs\n",
    ...
    f <- local({
      ...
      .dados2 <- dados2; .seed <- seed_exec * 1000 + ki * 100
      function(x) {
        inc <- which(x == 1)
    ...
    cat(sprintf("    [TIMER] GA: %.1f s | SNPs GA: %d\n",
    ...
    if (exec == 1) {
      ...
      print(
        ggplot(df_ga, aes(x = Geracao, y = Aptidao, group = Estatistica)) +
          ...
          labs(title  = sprintf("GA – F1 Minoria | %s | Exec 1 | Oliveira 2015 Sim6", nome),
...
# ==============================================================================
# SEÇÃO 5 – AGREGAÇÃO: UNIÃO POR KERNEL (10 execuções) — COMPARABLE A OLIVEIRA
# ==============================================================================

cat("\n")
cat(strrep("=", 70), "\n")
cat("  RESUMO FINAL — UNIÃO POR KERNEL (10 EXECUÇÕES)\n")
cat("  Estrutura equivalente a Oliveira (2015), Seção 8.8\n")
cat(strrep("=", 70), "\n\n")
...
for (nome in nomes_kernels) {
  # União dos SNPs GA das 10 execuções para este kernel
...
cat("\n--- Tabela resumo (União 10 execuções por kernel) ---\n")
print(resultados_uniao, row.names = FALSE)

# União de todos os kernels e todas as execuções (super-união)
snps_super_uniao   <- Reduce(union,     snps_uniao_por_kernel)
snps_super_intersec <- Reduce(intersect, snps_uniao_por_kernel)

cat("\n--- Super-União (todos os kernels × 10 execuções) ---\n")
metricas_snps(snps_super_uniao)

cat("\n--- Super-Interseção (todos os kernels × 10 execuções) ---\n")
...
# ==============================================================================
# SEÇÃO 6 – RESULTADOS POR EXECUÇÃO (detalhamento)
# ==============================================================================

cat("\n--- Detalhamento por execução (GA, após refinamento) ---\n")
...
write.csv(resultados_uniao,
          file = "Resultados_Uniao_10exec.csv", row.names = FALSE)
write.csv(resultados_por_exec,
          file = "Resultados_Detalhados_10exec.csv", row.names = FALSE)

# SNPs da super-união com indicação de causalidade
write.csv(
  data.frame(SNP    = snps_super_uniao,
             causal = snps_super_uniao %in% snps_causais_verdadeiros),
  file = "SNPs_super_uniao_10exec.csv", row.names = FALSE
)
write.csv(
  data.frame(SNP    = snps_super_intersec,
             causal = snps_super_intersec %in% snps_causais_verdadeiros),
  file = "SNPs_super_intersecao_10exec.csv", row.names = FALSE
)

# SNPs por kernel (união das 10 exec)
for (nome in nomes_kernels) {
  snps_k <- snps_uniao_por_kernel[[nome]]
  write.csv(
    data.frame(SNP    = snps_k,
               causal = snps_k %in% snps_causais_verdadeiros),
    file = sprintf("SNPs_uniao_%s_10exec.csv", nome), row.names = FALSE
  )
}

# ==============================================================================
# SEÇÃO 8 – TEMPOS DE EXECUÇÃO
# ==============================================================================

t_total <- round((proc.time() - t_total_inicio)[["elapsed"]])

cat("\n===== Tempos de Execução Detalhados (segundos) =====\n")
cat(sprintf("  %-30s: %6.1f s (média/exec: %.1f s)\n",
            "Simulação (total 10 exec)", sum(t_sim_vec), mean(t_sim_vec)))
cat(sprintf("  %-30s: %6.1f s (média/exec: %.1f s)\n",
            "Random Forest (total 10 exec)", sum(t_rf_vec), mean(t_rf_vec)))
cat("\n  -- Corte por kernel (soma das 10 execuções) --\n")
for (nome in nomes_kernels)
  cat(sprintf("  Corte %-20s: %6.1f s (média: %.1f s)\n",
              nome, sum(t_corte_mat[, nome]), mean(t_corte_mat[, nome])))
cat("\n  -- GA por kernel (soma das 10 execuções) --\n")
for (nome in nomes_kernels)
  cat(sprintf("  GA    %-20s: %6.1f s (média: %.1f s)\n",
