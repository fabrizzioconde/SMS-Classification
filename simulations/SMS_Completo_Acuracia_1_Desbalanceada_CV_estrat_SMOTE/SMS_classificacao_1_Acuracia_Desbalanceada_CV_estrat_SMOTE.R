...
list.snp <- list(1,2,3,4,5,6,7,8)    # SNPs CAUSAIS (índices das colunas)
list.ia  <- list(1,2,3,-1,-2,-3,1,2) # padrão de interações entre causais (ver scrime::simulateSNPglm)
beta     <- rep(2, 8)                # tamanho do efeito (igual para os 8 causais)
...
# Observação: usamos sempre 'head(names(rank_RF), N)' nos cortes abaixo (em vez
# de 'names(rank_RF)[1:N]') para evitar NAs quando o buffer pedir N > length.
...
for (cont in 1:(limite + 1)) {
  ...
  acc_mean <- svm_cv[1]                          # 1ª posição do vetor nomeado = acc_mean
  mean_svm_RF_list[[i]][cont] <- acc_mean         # armazena na posição 'cont' da trilha 'i'
}
...
plot(seq(passo, limite * passo + 10, by = passo),  # eixo X: nº de SNPs em cada ponto
     mean_svm_RF_list[[i]],                         # eixo Y: acurácia média da CV
...
f <- function(x) {
  ...
  acc_mean <- res[1]                        # acurácia média do conjunto candidato
  return(acc_mean)                          # GA maximiza por padrão -> usa diretamente
}
...
for (cont in 1:(limite + 1)) {
  ...
  acc_mean <- svm_cv[1]                          # acurácia média
